...
Imposition of pressure difference condition: dp = 0 
...
def solve_NS_monolithic(mesh, bmf, surface_marker, left_marker, right_marker, bottom_marker, w0, k, mu, rho, sigma, A, f, P_in, P_out):
    ...
    # U_tc = DirichletBC(W.sub(0), Constant((0.12, 0.0)), bmf, left_marker)
    # bcu = [symmetry, U_tc]
    ...
    L1 = - (P_in)*dot(n,v)*ds(left_marker) - (P_out)*dot(n,v)*ds(right_marker) # + inner(rho*g,v)*dx()
...
# ------------------------------------------------------------------------------#
sim_attempt = "microscale_sigma_072_dp_0/"
...
while t <= Time and dt > 0.0:
    ...
    k = Constant(dt)
    w_, no_iterations, converged, kappa, n_surf = solve_NS_monolithic(mesh, bmf, surface_marker, left_marker, right_marker, bottom_marker, w_n, k, mu, rho, sigma, A_, f, P_in, P_out)
